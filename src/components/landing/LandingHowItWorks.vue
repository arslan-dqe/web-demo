...
<template>
  <section id="how" class="py-28 px-4 relative overflow-hidden bg-(--ui-bg-muted)">
        ...
        >
          ...
          <div class="md:[direction:ltr]">
            ...
            <div class="relative">
              ...
              <p class="text-(--ui-text-muted) leading-relaxed mb-6 text-base">{{ step.desc }}</p>
              ...
              <ul class="space-y-2">
                ...
                >
                  <div class="w-4 h-4 rounded-full bg-primary/15 flex items-center justify-center flex-shrink-0">
                    <UIcon name="i-lucide-check" class="w-2.5 h-2.5 text-primary" />
                  </div>
                  {{ point }}
                </li>
              </ul>
            ...
            <div class="relative rounded-2xl border border-(--ui-border) bg-(--ui-bg) overflow-hidden shadow-2xl shadow-black/5 hover:-translate-y-1 transition-transform duration-500">
              ...
              <div class="p-5">
                ...
                <slot :name="`step-visual-${i}`">
                  ...
                  <!-- Step 0: Connect -->
                  <div v-if="i === 0" class="space-y-3">
                    <p class="text-xs text-(--ui-text-muted) font-medium mb-4">Choose your CRM integration</p>
                    <div
                    ...
                    >
                      ...
                      <UBadge size="xs" color="success" variant="soft" label="Connected" />
                    </div>
                  ...
                  <!-- Step 1: AI Scan -->
                  <div v-else-if="i === 1" class="space-y-3">
                    <div class="flex items-center justify-between mb-2">
                      <p class="text-xs text-(--ui-text-muted) font-medium">Scanning 48,291 records…</p>
                    ...
                    >
                      ...
                      <div class="h-1.5 w-full rounded-full bg-(--ui-bg-accented) overflow-hidden">
                        <div class="h-full rounded-full" :class="scan.color" :style="{ width: scan.pct + '%' }" />
                      </div>
                    </div>
                  ...
                  <div v-else class="space-y-3">
                    <div class="flex items-center justify-between mb-1">
                      <p class="text-xs text-(--ui-text-muted) font-medium">Review suggestions</p>
                    ...
                    >
                      <div class="flex items-center gap-2">
                        ...
                        <div class="w-5 h-5 rounded-full bg-primary/15 flex items-center justify-center flex-shrink-0">
                          <UIcon name="i-lucide-git-merge" class="w-3 h-3 text-primary" />
                        </div>
                      ...
                      <div class="flex items-center gap-2">
                        <div class="flex-1 h-1 rounded-full bg-(--ui-bg-accented) overflow-hidden">
                          <div class="h-full bg-gradient-to-r from-primary to-secondary rounded-full" :style="{ width: pair.score + '%' }" />
                        </div>
                        <span class="text-[10px] font-bold text-primary">{{ pair.score }}% match</span>
                      </div>
                      <div class="flex gap-2 pt-1">
                        <UButton size="xs" variant="soft"  color="success" label="Merge" leading-icon="i-lucide-check" class="flex-1" />
                        <UButton size="xs" variant="ghost" color="error"   label="Skip"  leading-icon="i-lucide-x"     class="flex-1" />
                      </div>
                    </div>
                  </div>
          ...
          >
            ...
            <div class="w-6 h-6 rounded-full border-2 border-primary/30 bg-(--ui-bg-muted) flex items-center justify-center">
              <UIcon name="i-lucide-chevron-down" class="w-3 h-3 text-primary/60" />
            </div>
          </div>
