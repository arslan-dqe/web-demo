      ...
      <div class="flex flex-col">
        ...
        <div class="relative px-6 pt-6 pb-5 border-b border-(--ui-border)">
          ...
          <Transition name="fade" mode="out-in">
            <div v-if="!submitted" key="h-active">
          ...
          <!-- Step dots -->
          <div v-if="!submitted" class="flex items-center gap-2 mt-4">
            <div
              v-for="s in 2" :key="s"
        ...
        <div class="px-6 py-6">
          <Transition name="step-slide" mode="out-in">
            ...
            <!-- Step 1: Email ─────────────────────────────────────── -->
            <div v-if="step === 1 && !submitted" key="s1" class="space-y-4">
              ...
              <div class="grid grid-cols-2 gap-3">
                ...
                <button class="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border border-(--ui-border) bg-(--ui-bg) hover:bg-(--ui-bg-elevated) hover:border-primary/30 transition-all text-sm font-medium text-(--ui-text)">
                  <svg class="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M21.6 12.2c0-.7-.1-1.4-.2-2H12v3.8h5.4c-.2 1.3-1 2.4-2.1 3.1v2.6h3.4c2-1.8 3.1-4.5 2.9-7.5z" fill="#4285F4"/>
                    <path d="M12 22c2.7 0 5-.9 6.7-2.4l-3.3-2.5c-.9.6-2 1-3.4 1-2.6 0-4.8-1.8-5.6-4.2H3v2.6C4.8 19.8 8.1 22 12 22z" fill="#34A853"/>
                    <path d="M6.4 13.9c-.2-.6-.3-1.2-.3-1.9s.1-1.3.3-1.9V7.5H3C2.4 8.7 2 10.3 2 12s.4 3.3 1 4.5l3.4-2.6z" fill="#FBBC05"/>
                    <path d="M12 5.4c1.5 0 2.8.5 3.8 1.5l2.9-2.9C16.9 2.4 14.6 1.4 12 1.4 8.1 1.4 4.8 3.6 3 7l3.4 2.6c.8-2.4 3-4.2 5.6-4.2z" fill="#EA4335"/>
                  </svg>
                  <span>Microsoft</span>
                </button>
              </div>
              ...
              <Transition name="fade">
                <div v-if="state.email && isValidEmail && !emailError">
                  ...
                  >
                    <UIcon name="i-lucide-alert-triangle" class="w-3.5 h-3.5 flex-shrink-0" />
                  ...
                  >
                    <UIcon name="i-lucide-check-circle-2" class="w-3.5 h-3.5 flex-shrink-0" />
            ...
            <!-- Step 2: Details ───────────────────────────────────── -->
            <div v-else-if="step === 2 && !submitted" key="s2" class="space-y-4">

              <!-- Email chip -->
              <div class="flex items-center gap-2 px-3 py-2 rounded-xl bg-(--ui-bg-elevated) border border-(--ui-border) text-sm">
                <UIcon name="i-lucide-mail" class="w-4 h-4 text-primary flex-shrink-0" />
                <span class="text-(--ui-text) truncate flex-1">{{ state.email }}</span>
                <button class="text-xs text-primary hover:underline flex-shrink-0" @click="step = 1">
                  Change
                </button>
              </div>
              ...
              <UFormField label="Company name" required :error="errors.company">
                <UInput
            ...
            <!-- Success ────────────────────────────────────────────── -->
            <div v-else-if="submitted" key="success" class="flex flex-col items-center text-center py-4">
              ...
              <div class="w-full space-y-2 mb-6 text-left">
                ...
                >
                  <div class="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center flex-shrink-0">
                    <UIcon :name="next.icon" class="w-4 h-4 text-primary" />
                  </div>
                  <div>
                    <div class="text-sm font-semibold text-(--ui-text-highlighted)">{{ next.label }}</div>
                    <div class="text-xs text-(--ui-text-muted)">{{ next.sub }}</div>
                  </div>
